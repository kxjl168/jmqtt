function test(){
	...
	var url = getRPath()+"/manager/moneymonthstastic/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/moneymonthstastic/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/moneymonthstastic/moneymonthstasticList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
		...
		queryParams : function queryParams(params) { // 设置查询参数
			var param = {
				...
				dayTime : $("#q_dayTime").val(),
				moneyType : $("#q_moneyType").val(),
		...
		columns : [ {
		 ...
		 {
				field : 'moneyType',
				title : '金额类型, 1:押金，2：消费，3:退款，4:充值',
				align : 'center',
				valign : 'middle',
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/moneymonthstastic/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/moneymonthstastic/delete";
