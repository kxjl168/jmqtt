function test(){
	...
	var url = getRPath()+"/manager/crsuduser/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/crsuduser/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/crsuduser/crsuduserList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
		...
		queryParams : function queryParams(params) { // 设置查询参数
			var param = {
				...
				userPhone : $("#q_userPhone").val(),
				userToken : $("#q_userToken").val(),
		...
		columns : [ {
		 ...
		 {
				field : 'baseDay',
				title : '用于计算过期时间的基础天数：(时间戳/1000/60/60/24)',
				align : 'center',
				valign : 'middle',
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/crsuduser/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/crsuduser/delete";
