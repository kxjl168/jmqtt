function test(){
	...
	var url = getRPath()+"/manager/test/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/test/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/test/testList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/test/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/test/delete";
