function test(){
	...
	var url = getRPath()+"/manager/rule/test";
...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/rule/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/rule/ruleList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
		...
		queryParams : function queryParams(params) { // 设置查询参数
			var param = {
				...
				createUserId : $("#q_createUserId").val(),
				dataType : $("#q_dataType").val(),
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/rule/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/rule/delete";
