...
/**
 * @author: chenenqiang
 * @createDate: 2018/11/6 09:06
 * @description:
 */
@Slf4j
public class DecodeUtil {

    public static String domainName = "http://api.heclouds.com";
     ...
     * @return token检验成功返回true；失败返回false
     */
...
//        log.debug("Token验证: result {},  接收 sig:{},   计算 sig:{}", sig.equals(signature.replace(' ', '+')), signature, sig);
     ...
     * @param obj   消息体对象
     * @param token OneNet平台配置页面token的值
     * @return 数据加密检验成功返回true；失败返回false
     */
...
//        log.debug("数据加密验证: result:{},  接收 sig:{},  计算 sig: {}", calSig.equals(obj.getMsgSignature()), obj.getMsgSignature(), calSig);
