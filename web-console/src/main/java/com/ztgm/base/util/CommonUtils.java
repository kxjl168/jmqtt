...
public class CommonUtils {
    ...
    public static Method getMethodByName(Class<?> clazz, String methodName) {
        ...
        // 入参不能为空
        if (null == clazz) {
            logger.info("CommonUtils.getMethodByName 入参错误，clazz：" + clazz + " ；methodName：" + methodName);
            return method;
        }
    ...
    public static <K, V> void listGroup2Map(List<V> list, Map<K, List<V>> map, Method method) {
        ...
        if (null == list || null == map || null == method) {
            logger.info("CommonUtils.listGroup2Map 入参错误，list：" + list + " ；map：" + map + " ；method：" + method);
            return;
        }
