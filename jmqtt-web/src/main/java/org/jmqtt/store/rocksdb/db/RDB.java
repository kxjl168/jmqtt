...
public class RDB {
	...
	public List<byte[]> getByPrefix(final ColumnFamilyHandle cfh, final byte[] prefixKey) {
		...
		try {
			...
			log.debug("[RocksDB] -> succ while get by prefix,columnFamilyHandle:{}, prefixKey:{}", cfh.toString(),
					new String(prefixKey));
		} catch (Exception e) {
	...
	public boolean deleteByPrefix(final ColumnFamilyHandle cfh, final byte[] prefixKey) {
		try {
			...
			log.debug("[RocksDB] -> succ while delete by prefix,columnFamilyHandle:{}, prefixKey:{}, nums:{}",
	...
	public boolean deleteRange(final ColumnFamilyHandle cfh, final byte[] beginKey, final byte[] endKey) {
		try {
			...
			log.debug("[RocksDB] -> succ delete range, columnFamilyHandle:{}, beginKey:{}, endKey:{}", cfh.toString(),
	...
	public byte[] get(final ColumnFamilyHandle cfh, final byte[] key) {
		try {
			...
			log.error("[RocksDB] -> error while get, columnFamilyHandle:{}, key:{}, err:{}", cfh.toString(),
	...
	public boolean delete(final ColumnFamilyHandle cfh, final byte[] key) {
		try {
			...
			log.error("[RocksDB] -> while delete key, columnFamilyHandle:{}, key:{}, err:{}", cfh.toString(),
					new String(key), e.getMessage(), e);
		}
		return true;
	}
	...
	public boolean put(final ColumnFamilyHandle cfh, final WriteOptions writeOptions, final byte[] key,
		...
		try {
			...
			log.error("[RocksDB] -> error while put, columnFamilyHandle:{}, key:{}, err:{}", cfh.isOwningHandle(),
					new String(key), e.getMessage(), e);
			return false;
		}
		return true;
	...
	public void init() {
		this.DB_OPTIONS.setCreateIfMissing(true).setCreateMissingColumnFamilies(true)
		...
		BLOCK_BASED_TABLE_CONFIG.setFilter(BLOOM_FILTER).setCacheIndexAndFilterBlocks(true)
				.setPinL0FilterAndIndexBlocksInCache(true);

		COMPRESSION_TYPES.addAll(Arrays.asList(CompressionType.NO_COMPRESSION, CompressionType.NO_COMPRESSION,
		...
		COLUMN_FAMILY_OPTIONS.setTableFormatConfig(BLOCK_BASED_TABLE_CONFIG)
				...
				.setLevel0StopWritesTrigger(10).setCompressionPerLevel(COMPRESSION_TYPES)
	...
	private List<ColumnFamilyDescriptor> getCFDescriptors() {
		...
		// zj add
		list.add(new ColumnFamilyDescriptor(RDBStorePrefix.RULE_MESSAGE.getBytes(Charset.forName("UTF-8"))));
		list.add(new ColumnFamilyDescriptor(RDBStorePrefix.IOTOBJECTMODEL_MESSAGE.getBytes(Charset.forName("UTF-8"))));
		list.add(new ColumnFamilyDescriptor(RDBStorePrefix.MSG_FEE_LOG.getBytes(Charset.forName("UTF-8"))));
