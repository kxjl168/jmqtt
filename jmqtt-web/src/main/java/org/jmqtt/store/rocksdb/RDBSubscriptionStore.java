...
public class RDBSubscriptionStore implements SubscriptionStore {
	...
	public boolean storeSubscription(String clientId, Subscription subscription) {
		try {
			this.rdb.putSync(columnFamilyHandle(), key(clientId, subscription.getTopic()),
					SerializeHelper.serialize(subscription));
			return true;
		} catch (Exception ex) {
	...
	private byte[] key(String topic) {
		return (RDBStorePrefix.SUBSCRIPTION  + topic).getBytes(Charset.forName("UTF-8"));
	}
