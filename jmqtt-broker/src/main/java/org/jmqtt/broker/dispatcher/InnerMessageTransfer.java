...
public class InnerMessageTransfer implements ClusterMessageTransfer {
	 ...
	 * @date 2020年2月13日
	 */
	public void dispatcherOtherMessage2Cluster(int requestCode, Message message) {
		ClusterRemotingCommand remotingCommand = new ClusterRemotingCommand(requestCode);
	...
	private class InnerMessageListener implements MessageListener {
		...
		private InnerMessageListener() {
			this.clusterMessageExecutor = new ThreadPoolExecutor(8, 8, 60000, TimeUnit.MILLISECONDS,
					new LinkedBlockingQueue<>(10000), new ThreadFactoryImpl("ClusterMessageExecutor"));
		}
	...
	private void dispatcherMessage2Client(ClientSession clientSession, Message message) {
		...
		MqttPublishMessage publishMessage = MessageUtil.getPubMessage(message, false, qos,
				clientSession.generateMessageId());
	...
	private void loadRemoteSession(ClusterRemotingCommand command) {
		...
		for (Subscription subscription : subscriptions) {
			...
			if (clientSession == null) {
				...
				if (state == null) {
					connLog.warn("cluster transfer session this node has no session of this client,clientId={}",
							clientId);
					return;
				}
			}
	...
	private void transferOfflineMessage(String clientId, String originNode) {
		...
		for (Message message : flowSendMessages) {
			...
			byte[] body = SerializeHelper.serialize(message);
			ClusterRemotingCommand remotingCommand = new ClusterRemotingCommand(
					ClusterRequestCode.TRANSFER_SESSION_MESSAGE, body);
			send(originNode, remotingCommand);
		}
		this.flowMessageStore.clearClientFlowCache(clientId);
